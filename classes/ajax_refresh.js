...
function refreshStats() {
	$.ajax({
		...
		success: function (data) {
			...
			$('#diskbar').css('width', data.DiskPercent + '%');
			$('#diskpercent').attr('src', "classes/bar.php?rating=" + data.DiskPercent);
			$('#cpubar').css('width', data.CPUPercent + '%');
			$('#cpupercent').attr('src', "classes/bar.php?rating=" + data.CPUPercent);
