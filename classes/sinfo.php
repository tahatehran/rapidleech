...
$prozent_belegt = 100 * $belegt / $insgesamt;
	if ($os == "windows") {
		...
		$cpu_string .= '%<div class="meter"><span style="width:' . round ( $cpu_load, "2" ) . '%;"></span></div>';
	} elseif ($os == "linux") {
		...
		if (($cpu = getCpuUsage()) === -1) { $cpu_string = -1; }
			...
			$cpu_string .= lang(136).": <span id='cpuload'>" . round ( $cpulast, "0" ) . "</span>%";
			$cpu_string .= '<div class="meter"><span id="cpubar" style="width:' . round ( $cpulast, "2" ) . '%;"></span></div>';
		}
	} elseif ($os == "nocpu") {
		$cpu_string = '';
	} else {
