...
class mega_co_nz extends DownloadClass {
	...
	private function cJar_load($user, $pass, $filename = 'mega_dl.php') {
		...
		if (!file_exists($filename) || !($savedcookies = file($filename)) || !is_array($savedcookies = unserialize($savedcookies[1], array('allowed_classes' => false)))) return $this->Login($user, $pass);
	...
	private function cJar_save($user, $pass, $filename = 'mega_dl.php') {
		...
		$filename = DOWNLOAD_DIR . basename($filename);
		if (file_exists($filename) && ($savedcookies = file($filename)) && is_array($savedcookies = unserialize($savedcookies[1], array('allowed_classes' => false)))) {
			// Remove old cookies
