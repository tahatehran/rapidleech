...
class rapidgator_net extends DownloadClass {
	...
	private function cJar_load($user, $pass) {
		...
		if (file_exists($this->cJar['file']) && ($cFile = file($this->cJar['file'])) && is_array($cFile = unserialize($cFile[1], array('allowed_classes' => false))) && array_key_exists($this->cJar['hash'], $cFile) && ($testCookie = $this->cJar_decrypt($cFile[$this->cJar['hash']]['cookie'], $this->cJar['key']))) {
	...
	private function cJar_save() {
		...
		if (file_exists($this->cJar['file']) && ($savedcookies = file($this->cJar['file'])) && is_array($savedcookies = unserialize($savedcookies[1], array('allowed_classes' => false)))) {
			// Remove old cookies
